import { GroupRecordsTypeEnum, type GroupedRecords } from "@/types/GroupedRecordsTypes";
...
function groupRecords(records: Record[]): GroupedRecords[] {
  ...
  const groupedRecordsObj = records.reduce((acc: { [key: string]: Record[] }, record: Record) => {
  ...
  const groupedRecordsArray: GroupedRecords[] = Object.keys(groupedRecordsObj).map((groupTitle: string) => {
