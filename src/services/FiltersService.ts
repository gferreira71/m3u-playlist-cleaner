...
function applyFilters(groupedRecords: GroupedRecords[], filtersToApply: RecordsFitlers): GroupedRecords[] {
            ...
            passesTextFilter = filtersToApply.scopeText.some((scope) => {
                switch (scope) {
                    case ScopeTextEnum.NAME:
                        return record.name && record.name.toUpperCase().includes(textToSearch);
                    case ScopeTextEnum.TVGID:
                        return record.tvgParameters?.tvgId && record.tvgParameters.tvgId.toUpperCase().includes(textToSearch);
                    case ScopeTextEnum.TVGNAME:
                        return record.tvgParameters?.tvgName && record.tvgParameters.tvgName.toUpperCase().includes(textToSearch);
                    case ScopeTextEnum.GROUPTITLE:
                        return group.groupTitle && group.groupTitle.toUpperCase().includes(textToSearch);
                    default:
                        return false;
                }
            });
        } else {
            passesTextFilter = true; // If no text filter provided, consider it as passed
        }

        const passesScopeFilter = filtersToApply.scope.length === 0 || (record.type && filtersToApply.scope.includes(record.type));
